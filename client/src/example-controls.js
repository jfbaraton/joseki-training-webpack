...
let startPathNode = null; // updated when we reset to a "startPath" position. used to calculate the rootnode stats
...
const _updateStartNodeStats = function (currentNode, controls) {
    if(currentNode) {
...
const ExampleGameControls = function(element, game) {
    ...
    this.postEngineCmd = function(cmd, handleResponse) {
        const prevLength = [0];
        axios
            .post("/api/engine", {
                cmd:cmd
                ...
                onDownloadProgress: progressEvent => {
                    ...
                    console.log('POST chunk ',progressEvent.currentTarget.response.length);
                   ...
                   progressEvent && progressEvent.currentTarget && handleResponse && handleResponse(progressEvent.currentTarget.response.substr(prevLength[0]));
                   prevLength[0] = progressEvent.currentTarget.response.length;
                   if(prevLength > 20000) {
                        controls.postEngineCmd("\n"); // interrupts kata analyze
                   }
                }
            })
            //--- read chunck
